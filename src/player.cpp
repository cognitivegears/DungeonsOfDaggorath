...
// Constructor
Player::Player()
    : PPOW(PLRBLK.P_ATPOW), PMGO(PLRBLK.P_ATMGO), PMGD(PLRBLK.P_ATMGD),
      PPHO(PLRBLK.P_ATPHO), PPHD(PLRBLK.P_ATPHD), PDAM(PLRBLK.P_ATDAM),
      turnDelay(20), moveDelay(25), wizDelay(500) {
  Reset();
}
...
int Player::PLAYER() {
  ...
  scheduler.TCBLND[Scheduler::TID_PLAYER].next_time =
      scheduler.curTime + scheduler.TCBLND[Scheduler::TID_PLAYER].frequency;
...
int Player::HSLOW() {
  ...
  // Update Task's next_time
  scheduler.TCBLND[Scheduler::TID_HRTSLOW].next_time =
      scheduler.curTime + (HEARTR * 17);
...
void Player::HUPDAT() {
  ...
  //
  HEARTR =
      (((PLRBLK.P_ATPOW) * 64) / ((PLRBLK.P_ATPOW) + ((PLRBLK.P_ATDAM) * 2))) -
      18;
...
int Player::BURNER() {
  ...
  scheduler.TCBLND[Scheduler::TID_TORCHBURN].next_time =
      scheduler.curTime + scheduler.TCBLND[Scheduler::TID_TORCHBURN].frequency;
...
void Player::setInitialObjects(bool isDemo) {
  ...
  if (isDemo) {
    game.IsDemo = true;
    game.LEVEL = 2;
    dungeon.SetLEVTABOrig(); // Make sure the original seeds aren't overwritten
                             // from pervious new game.
    ...
    if (!game.RandomMaze)      // Do we have an original maze?  Yes:
      dungeon.SetLEVTABOrig(); // Make sure the original seeds aren't
                               // overwritten from pervious new game.
...
void Player::PATTK() {
  ...
  if (U->obj_id >= Object::OBJ_RING_ENERGY &&
      U->obj_id <= Object::OBJ_RING_FIRE) {
  ...
  if (DAMAGE(PLRBLK.P_ATPOW, PLRBLK.P_ATMGO, PLRBLK.P_ATPHO,
             creature.CCBLND[cidx].P_CCPOW, creature.CCBLND[cidx].P_CCMGD,
  ...
  if (creature.CCBLND[cidx].creature_id != Creature::CRT_WIZARD) {
    ...
    if (game.CreaturesInstaRegen)
      creature
          .CREGEN(); // Regen creature intantly after death if option selected.
    return;
  }
...
bool Player::DAMAGE(int AP, int AMO, int APO, int DP, int DMD, int DPD,
                    dodSHORT *DD) {
  int a;
  ...
  if (res <= 0) {
    ...
    if (A == Parser::DIR_UP) {
      ...
      if (vres == Dungeon::VF_LADDER_UP ||
          ((vres == Dungeon::VF_HOLE_UP) &&
           creature.FRZFLG)) // can only clumb up pits after you win
      {
...
void Player::PREVEA() {
  ...
  if (req && ((req * 25 <= PPOW) || (g_cheats & CHEAT_REVEAL) ||
    ...
    for (x = 0; x < lines; ++x) {
      ...
      do {
        ...
        if (redraw) {
          ...
          viewer.drawVector((x * inc * dir) + offset, y0,
                            (x * inc * dir) + offset, y1);
